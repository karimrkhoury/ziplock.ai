...
const getWhatsAppMessage = (downloadUrl: string, password: string, lang: Language) => {
  ...
  const shareUrl = downloadUrl || window.location.href;
...
const getEmailMessage = (downloadUrl: string, password: string, lang: Language) => {
  // Use the provided downloadUrl directly - it should be properly formatted from App.tsx
  // If it's not available, use the current URL as fallback
  const shareUrl = downloadUrl || window.location.href;
...
const CompletedView: React.FC<CompletedViewProps> = ({
  ...
  const handleCopyLink = () => {
    // Use the downloadUrl directly - it should be properly formatted from App.tsx
    // If it's not available, use the current URL as fallback
      ...
      <div className="w-full max-w-[500px] mx-auto space-y-3 sm:space-y-6">
        ...
        <div className="flex justify-between items-center w-full max-w-[280px] sm:max-w-[320px] mx-auto">
          ...
          <button
            onClick={() => {
              try {
                // Create a shorter message for WhatsApp to avoid character limits
                const lang = translations.missionAccomplished.title.match(/[\u0600-\u06FF]/) !== null ? Language.AR : Language.EN;
                const shareUrl = downloadUrl || window.location.href;
                
                // Create a simpler message for WhatsApp
                let whatsappText;
                if (lang === Language.AR) {
                  whatsappText = `ملفات مشفرة بأمان!\n\nالرابط: ${shareUrl}\n\nكلمة المرور: ${password}\n\nتم إنشاؤها باستخدام ziplock.me`;
                } else {
                  whatsappText = `Securely encrypted files!\n\nLink: ${shareUrl}\n\nPassword: ${password}\n\nCreated with ziplock.me`;
                }
